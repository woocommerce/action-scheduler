...
Any change to a **public or externally exposed** class, interface, function, or method signature is **high-risk** and **must state its backward-compatibility impact in the PR description**. The declared public API is the top-level functions in ./functions.php — `as_enqueue_async_action()`, `as_schedule_single_action()`, `as_schedule_recurring_action()`, `as_schedule_cron_action()`, `as_unschedule_action()`, `as_unschedule_all_actions()`, `as_next_scheduled_action()`, `as_has_scheduled_action()`, `as_get_scheduled_actions()`, `as_get_datetime_object()` and `as_supports()` — but the surface that actually has consumers is much wider, per the Structure section above. Note that the parameters added to those functions over time (`$unique`, `$priority`) are all optional and trailing: that is the shape a safe change to a published signature takes.
...
**Multiple versions coexist and the newest one wins.** Per the Bootstrapping section above, whichever embedded copy is most recent is the one that loads. So a change ships to host plugins that never adopted it, and calls arrive from callers written against much older versions. Two consequences: what you remove can still be called by an older host plugin, and what you add is not guaranteed to exist for a consumer whose site loaded an older copy. `as_supports()` is the escape hatch for the second case. There is none for the first, which is why removal is almost never the right move.

**Adding an abstract method to a base class that external code extends must be flagged explicitly.** `ActionScheduler_Store`, `ActionScheduler_Logger`, `ActionScheduler_Lock` and `ActionScheduler_Abstract_QueueRunner` are abstract classes rather than interfaces, but they carry the same risk: a new `abstract public function` fatals every third-party subclass on load, because it no longer satisfies the contract. `ActionScheduler_Schedule` is a true interface and the rule applies to it directly. Removing a required method is breaking too — existing implementers are left carrying a now-dead method that their static analysis will flag. Prefer a non-breaking alternative: add a concrete default implementation on the base class rather than an abstract declaration, or introduce a separate new interface.

**Those base classes are swappable by filter, which is what makes them contracts.** `action_scheduler_store_class`, `action_scheduler_logger_class`, `action_scheduler_queue_runner_class`, `action_scheduler_lock_class` and `action_scheduler_stored_action_class` let a site replace our concrete implementations with its own. Any change that assumes the concrete class is one of ours — a type check, a new method call, a property access — breaks those sites.

**Deprecate, don't rename.** For existing public symbols (classes, methods, constants, hooks), never rename or remove them in place. Mark the old symbol `@deprecated`, introduce the replacement alongside it, and keep both working through a deprecation window. ./deprecated/ and the `_deprecated_function()` calls that name the version they were introduced in are how this repo has always done it; extend that pattern rather than deleting.
...
**Hooks and filters are public contracts.** Every `do_action` and `apply_filters` call is an interface that third-party callbacks depend on, including the whole `action_scheduler_*` family and the `pre_as_*` short-circuit filters on the scheduling functions. Site operators use these in production to keep queues healthy — `action_scheduler_queue_runner_batch_size`, `action_scheduler_queue_runner_time_limit`, `action_scheduler_retention_period` — on sites that have never touched our code otherwise. Removing a hook, renaming it, or removing/reordering its arguments breaks every attached callback. Changing *when* or *whether* a hook fires can break consumers that depend on its timing, and here that can mean queued work silently stops running. Additive is the safe path: append new arguments at the end, never remove or reorder existing ones. To retire a hook, fire it through `do_action_deprecated()` / `apply_filters_deprecated()` for a deprecation window instead of deleting it.

**Never trust data that flows through hooks.** Keep hook callback parameters untyped and validate or coerce the value before passing it to strictly typed code, since any callback can receive a value another one produced. And when firing a filter, validate the final return value before using it: the class-name filters above are the sharpest example, as `action_scheduler_store_class` can return a class that does not exist or does not extend `ActionScheduler_Store`.

**Overridable classes are contracts too, including which internal methods get called.** Third parties subclass our stores and override individual methods — `query_actions()` most often. Adding a fast path or a direct query that bypasses an overridable method silently disables those overrides even though no signature changed: the extending code simply stops running. When optimizing a store, a logger or the runner, ensure overridable methods are still invoked on every code path, or treat the change as breaking.

**Stored data is a contract, and it outlives the code that wrote it.** Assume host plugins and site tooling read the custom tables (`{$wpdb->prefix}actionscheduler_actions`, `_claims`, `_groups`, `_logs`) directly, not only through the store API — and that unmigrated sites still hold rows in the legacy `scheduled-action` post type and `action-group` taxonomy. Renaming a column, narrowing a type, or changing what a `ActionScheduler_Store::STATUS_*` value means breaks those consumers. A schema change means bumping `$schema_version` in `ActionScheduler_StoreSchema` or `ActionScheduler_LoggerSchema` and shipping an upgrade path that leaves existing rows readable. Schedules are persisted as `serialize()`d objects and read back through `ActionScheduler_ScheduleDeserializer`, so renaming or removing a schedule class orphans every pending action an older version wrote: that is data loss, not a refactor.

**Do not assume global state.** Actions execute from WP Cron, from the async loopback request dispatched by `ActionScheduler_AsyncRequest_QueueRunner`, and from WP CLI — which is why `$context` is threaded through the runner methods. None of those set the globals a front-end request does (`$post`, `$wp_query`, an initialized session or cart), and the host plugin that bundled us may not be fully booted either. Nothing here may assume WooCommerce is present at all; it is integrated with when detected (`ActionScheduler_wcSystemStatus`), never depended on. A newly introduced read of a global in a path reachable from the runner is a fatal or a silent misbehavior in the contexts that do not set it. Guard the exact dependency explicitly: `function_exists`/`class_exists` for symbols, `isset` for variables, `did_action` for lifecycle state.

**Do not assume single-site.** Multisite changes where data lives: site-scoped vs network-scoped options (`get_option` vs `get_site_option`), per-site tables, user roles and capabilities, and upload paths all differ. Our tables are built from `$wpdb->prefix`, which is site-scoped, so each site on a network carries its own actions, claims, groups and logs tables, and each needs creating and upgrading independently. A change that reads or writes site state must state in its PR whether it behaves correctly under multisite — and if it was not tested there, say so explicitly.

**Do not assume install layout.** WordPress could be configured to run in a subdirectory, with relocated `wp-content`, and behind reverse proxies. On top of that, Action Scheduler is rarely installed as a plugin in its own right: it usually sits inside another plugin's directory, and can be loaded from a theme or an mu-plugin. Never build paths or URLs by concatenation, and never hardcode a location relative to the plugins directory — derive them via `ActionScheduler::plugin_path()` and `ActionScheduler::plugin_url()`, which resolve against the file that actually loaded us. A path that works when installed standalone and breaks when embedded is a compatibility bug, not an edge case.
