...
**Overridable classes are contracts too, including which internal methods get called.** The base classes listed above exist to be subclassed, and the `action_scheduler_*_class` filters mean a third-party subclass can be the live implementation on any site. Every public or protected method on them is a contract: changing its signature or removing it breaks those subclasses. Adding a fast path or skip that avoids calling an overridable method silently disables those overrides even though no signature changed: the subclass's code simply stops running. When optimizing such a class, ensure overridable methods are still invoked on every code path, or treat the change as breaking.

**Registered script and style handles are public contracts.** This library registers none today; keep it that way. The moment a handle ships, third-party code can enqueue it or list it as a dependency, and its name is frozen. To rename one later, register the legacy handle as an alias that depends on the new one (the same pattern WordPress core uses for `jquery` → `jquery-core`) rather than removing it; do not register the same file under both handles, or pages with mixed consumers will load it twice.

**Schema updates are one-shot and must stay compatible one version back.** Table changes ship by incrementing `$schema_version` in the schema classes; each version's update runs once and is marked complete, so never amend a shipped schema version - increment it again. A host plugin can be downgraded, leaving an older embedded copy running against tables a newer copy already upgraded, so every schema change must stay readable one version back. Additive columns and keys survive that; dropping or repurposing a column fatals or corrupts under code that still expects it.
